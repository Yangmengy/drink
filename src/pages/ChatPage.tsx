...
export function ChatPage() {
  const { messages, pending, pendingMessage, messagesLoading, loading, clearing, error, draft, setDraft, failed, send, pendingMode, configured, setConfigured, recommendLocal, localError, liveReply, activeConversationId } = useChat();
  const bar = useBar();
  ...
  async function searchLocal(input: LocalRecommendationInput) {
    if (pending || messagesLoading || loading || clearing) return;
    if (failed && error) setFailedTurnAnchor(messages.length);
    const request = failedTrace ? { ...input, afterTraceId: failedTrace } : input;
    awaitingLocalReply.current = true;
    followLatest.current = true;
    setLocalOpen(false);
    if (!await recommendLocal(request)) {
      awaitingLocalReply.current = false;
      setLocalOpen(true);
    }
  }
  ...
  function openClearConversation() {
  ...
  const renderMessage = (message: ChatMessage, index: number) => (
    ...
    >
      ...
      {message.traceId && <ChatTracePanel traceId={message.traceId} conversationId={activeConversationId} />}
    ...
    <section className="chat-page">
      <div className="chat-top" ref={chatTop}>
        <header className="page-header">
          ...
          <button
            ...
            aria-label="清空对话"
            title={messages.length ? '清空当前对话和上下文' : '暂无对话可清空'}
            disabled={loading || !!pending || clearing || !messages.length}
            onClick={openClearConversation}
          >
            <Trash2 size={19} />
          </button>
        </header>
      ...
      <div className="conversation" ref={conversation} role="log" aria-label="聊天记录" aria-live="polite">
        ...
        {failedAnchor !== null && (
          <div className="error archived" role="note">
            ...
            {failedTrace && <ChatTracePanel traceId={failedTrace} conversationId={activeConversationId} label="查看失败链路" />}
        ...
        {pending && (
          <>
            <article className="message user"><span className="message-author">我</span><p>{pendingMessage}</p></article>
            <article className="message assistant">
              <span className="message-author">{pendingMode === 'local' ? 'Bartender · 本地模式' : 'Bartender'}</span>
              {liveReply.traceId && <ChatTracePanel traceId={liveReply.traceId} conversationId={activeConversationId} liveEvents={liveReply.events} />}
              <div className="assistant-content">
      ...
      <div className="composer-dock">
        <form className="composer" onSubmit={submit}>
          ...
          />
          <button className="send-button" type="submit" disabled={!draft.trim() || !!pending || messagesLoading || loading || clearing} aria-label="发送消息"><ArrowUp size={16} /></button>
        </form>
        <p id="message-help" className="sr-only">{pendingMessage ? '正在等待回复。可以先写下一句，切换页面也会保留。' : configured === false ? '未连接聊天模型。上方可直接查询本地酒单。' : 'Enter 发送 · Shift + Enter 换行。配方来自你的酒单。'}</p>
      </div>
    </section>
  );
}
