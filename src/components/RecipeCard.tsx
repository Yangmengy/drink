  ...
  const card = <>
    <button className="recipe-card" onClick={() => setOpen(true)}>
      ...
      <div className="recipe-card-body">
        ...
        <span className={`availability ${!unavailable && recipe.canMake ? 'ready' : ''}`}>{unavailable || (recipe.canMake ? <><Check size={14} /> 材料齐了</> : recipe.missing.length ? `缺 ${recipe.missing.length} 种：${recipe.missing.join('、')}` : '配方待完善')}</span>
      </div>
    </button>
    <dialog ref={dialog} className="recipe-dialog" onClose={() => setOpen(false)} onClick={e => { if (e.target === e.currentTarget) setOpen(false); }} aria-label={`${recipe.name}配方`}>
      <div className="dialog-content">
        ...
        <p className="availability">{unavailable || (recipe.canMake ? '所需材料种类齐全，请确认剩余用量。' : `还缺：${recipe.missing.join('、') || '配方原料信息'}`)}</p>
        ...
        {recipe.source === 'custom' && inventoryStatus !== 'removed' && <Link className="button secondary" to={`/custom?id=${encodeURIComponent(recipe.id)}`} onClick={() => setOpen(false)}>编辑这款酒</Link>}
      </div>
    </dialog>
  </>;
